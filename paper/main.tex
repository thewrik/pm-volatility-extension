...
tractable quantiles obtained from a standard beta inverse CDF. We also replace the locally additive DR-AS variance with
...
inference. Across 1,007,560 out-of-sample hours, the replication confirms the
active-update DR--AS gain over deadline resolution. On the full panel MHB
improves volume-weighted tick log score over no-hurdle and post-hoc-coherence
ablations, with paired cluster intervals excluding zero, but it does not beat
the active-fit DR--AS normal benchmark on volume-weighted log or interval score.
The density advantage over DR--AS appears in Economics and reverses in Politics
and Sports. The result identifies both the value and the empirical limit of
martingale coherence without a scheduled-event or jump component.
\end{abstract}
...
Prices live on a market-specific lattice. Kalshi metadata identify cent,
deci-cent, and tapered price-level structures; a mid-quote has local grid width
equal to one-half the finest bid/ask tick at that observation. The primary log
score therefore uses the forecast probability of the observed realized-price
cell. For MHB this cell
...
randomized probability integral transforms, and equal-, volume-, and
contract-balanced aggregates. Contract-balanced weights normalize hourly volume
to sum to one within each contract, limiting domination by a few high-volume
claims. Contract-cluster resampling preserves within-contract dependence.
...
mid-quote. Market-level price ranges determine the score lattice. Only
consecutive observations exactly one hour apart are retained.
...
lattice but does not identify these mechanisms separately. Moreover, rounding a
latent martingale need not itself produce a martingale. Theorem~\ref{thm:coherence}
is exact for the price process being modeled; its empirical use on displayed
mid-quotes is therefore a maintained forecasting restriction, not an assertion
that market microstructure preserves the martingale property exactly.
...
no-change atom, bounded support, and proper density forecasts. The locked
evidence is deliberately narrower than a dominance claim. Coherence materially
improves density fit over both hurdle ablations, yet the active-fit DR--AS normal
benchmark remains stronger under the volume-weighted criterion, especially in
Politics and Sports. Thus the theorem is a necessary design restriction, not a
sufficient volatility model: event clocks, jump mixtures, or richer active-move
kernels are required before the unconditional extension can dominate in
event-heavy markets. That negative boundary is part of the contribution rather
than a result to be tuned away.
